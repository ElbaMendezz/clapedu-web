...
## Fix — parpadeo en la animación de números de "Impacto"/"Cifras" (2026-07-14)

El usuario reportó que la animación de conteo de números en la sección "Impacto" del home producía un parpadeo raro en el fondo. Causa raíz identificada: el fondo animaba `background-position` (propiedad que fuerza repintado, no compositor) en un loop CSS infinito de 16s, y ese repintado continuo competía por el mismo frame budget con el conteo de los números — `count-up.ts` actualiza `textContent` hasta 60 veces/seg durante 1.5s en 6 elementos a la vez — sobre una capa con 2 blobs difuminados (`filter: blur`) detrás. Sin ningún aislamiento de repintado entre los números y el fondo, cada frame del conteo forzaba también repintar el degradado y los blobs, lo que se percibía como parpadeo.

Fix aplicado en `src/components/sections/Impacto.astro` (home) y replicado en `src/components/sobre-nosotros/Cifras.astro` (misma estructura, mismo bug — sección "Cifras" de `/sobre-nosotros`, id `#impacto`):

- El degradado se movió de `background-position` sobre `section` a un pseudo-elemento `section::before` sobredimensionado (`inset: -20%`) animado con `transform: translate3d + scale` (compositor, no repinta). Keyframes con nombre propio por componente (`impacto-gradient-drift` / `cifras-gradient-drift`) en vez de reusar el `clap-gradient-shift` compartido de `global.css`, para no afectar a Hero/404/Contacto que también lo usan y no tenían el problema reportado.
- `.metric-number` gana `contain: content` (aísla el repintado del texto que cambia en cada frame del conteo, para que no se propague al fondo) y `font-variant-numeric: tabular-nums` (evita que los dígitos salten de ancho mientras cuentan). Requiere `display: inline-block` — `contain` no aplica a cajas inline.
- `.blob` gana `will-change: transform` para promoverse a su propia capa compositada y no re-rasterizar el `blur` en cada frame.
...
## Favicon: ícono ":p:" en vez del wordmark (2026-07-14)

El usuario agregó `public/clap-favico.png` (el isotipo ":p:" morado, 2309×1567) y pidió que fuera el nuevo favicon, reemplazando el wordmark de la entrada anterior. Al inspeccionar el archivo con `sharp` se encontró que el ícono real solo ocupa ~60%×55% del canvas (bbox 460,352 → 1846,1214 sobre 2309×1567) — el resto es relleno transparente. Igual que con el wordmark, se generó una vista previa a tamaño real (16×16 y 32×32) antes de aplicarlo: con el archivo tal cual, el ícono se ve notablemente más chico y borroso dentro de esos pocos píxeles. Se le mostró la comparación al usuario (recortado vs. tal cual) y eligió que se recorte ajustado al ícono.

Se generó `public/favicon.png` (nuevo archivo, 512×512, no reemplaza el original) recortando al bounding box real del contenido con ~6% de margen, cuadrando el canvas centrado y con fondo transparente, luego reescalado a 512px — resolución suficiente para que el navegador la reduzca con buen antialiasing a cualquier tamaño real de pestaña. `public/clap-favico.png` (el archivo que puso el usuario, sin recortar) se dejó tal cual en `/public`, sin borrar — es la fuente original, pero después de este cambio queda sin ninguna referencia en el sitio.

Se actualizó `src/layouts/BaseLayout.astro`: `<link rel="icon" type="image/png">` pasa de `/logo-clap-purple.png` a `/favicon.png`. La referencia de `/logo-clap-purple.png` en el JSON-LD (`organizationSchema.logo`) no se tocó — es un asset distinto con un propósito distinto. `apple-touch-icon.png` tampoco se tocó, mismo criterio que la vez anterior.

**Verificado:** `astro check` → 0/0/0 · `npm run build` → OK · HTML compilado contiene `<link rel="icon" type="image/png" href="/favicon.png">` · `curl` confirma `200` y MD5 idéntico entre lo servido y `public/favicon.png` · preview a 16px/32px del archivo final confirma que el ícono se lee claramente como ":p:".
